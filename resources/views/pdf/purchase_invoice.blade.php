...
<body>
    @php
        ...
        $rest = max(0,(float)($order->total_ttc ?? 0) - $paid);
    @endphp
    <div class="header">
        <div class="brand">
    ...
    <div class="meta">
        <div class="box">
            ...
            @if($order->supplier->phone)<div>Tél: {{ $order->supplier->phone }}</div>@endif
            @if($order->supplier->email)<div>Email: {{ $order->supplier->email }}</div>@endif
        </div>
        <div class="box">
            <div class="label">Détails</div>
            <div>Date: <strong>{{ optional($order->ordered_at)->format('Y-m-d') }}</strong></div>
    ...
    <div class="totals">
        <table>
            <tr>
              <td>Total HT</td>
              <td class="text-end">{{ number_format((float)($order->total_ht ?? $order->total_ttc),2,',',' ') }} GNF</td>
            </tr>
            <tr class="grand">
              <td>Total TTC</td>
              <td class="text-end">{{ number_format((float)$order->total_ttc,2,',',' ') }} GNF</td>
            </tr>
            <tr>
              <td>Payé</td>
